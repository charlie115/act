...
export default function GoogleSignInButton() {
  ...
  if (!googleClientId) {
    return (
      <button
        className="flex w-full items-center justify-center gap-2.5 rounded-xl border border-border bg-surface-elevated/60 px-4 py-3.5 text-sm font-semibold text-ink-muted transition-colors"
        disabled
        type="button"
      >
        <GoogleIcon />
  ...
  return (
    ...
    >
      <GoogleIcon />
      {isLoading ? "로그인 중..." : "Google 계정으로 계속"}
    </button>
  );
}
