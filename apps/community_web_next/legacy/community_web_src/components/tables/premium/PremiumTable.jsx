...
function PremiumTable({
  ...
  const assetsParam = useMemo(
    () => (realTimeAssets.length ? realTimeAssets.join(',') : undefined),
    [realTimeAssets]
  );

  const ready = realTimeAssets.length > 0;
  ...
  const { data: walletStatus } =
    useGetWalletStatusQuery(
      { baseAsset: assetsParam, ...marketCodes },
      {
  ...
  const { data: klineVolatilityData } = useGetKlineVolatilityQuery(
    ...
    { 
      pollingInterval: 1000 * 120, // Increased to 2 minutes
      skip: !ready || !assetsParam || !marketCodes,
      refetchOnFocus: false,
      refetchOnMountOrArgChange: false
    }
  );
  ...
  const prevAssets = usePrevious(realTimeAssets);
  ...
  useEffect(() => {
    if (!prevIsAssetsDataSuccess && isAssetsDataSuccess)
      if (!isEqual(prevAssets, realTimeAssets))
        realTimeAssets.forEach((asset) => {
          if (!assetsData?.[asset]) postAsset({ symbol: asset });
        });
  }, [assetsData, isAssetsDataSuccess, prevAssets, realTimeAssets]);
  ...
  useEffect(() => {
    if (marketCodes && lastMarketCodesRef.current && 
        (lastMarketCodesRef.current.targetMarketCode !== marketCodes.targetMarketCode ||
         lastMarketCodesRef.current.originMarketCode !== marketCodes.originMarketCode)) {
      
      setMarketCodeChangeCounter(prev => prev + 1);
    }
    
    lastMarketCodesRef.current = marketCodes ? { ...marketCodes } : marketCodes;
  }, [marketCodes]);
