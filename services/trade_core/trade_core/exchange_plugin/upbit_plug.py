...
def calculate_upbit_price(price):
    if price >= 2000000:  # 2,000,000원 이상
        ...
        price = int(price/5)*5 # 5원 단위
    elif price >= 1000:  # 1,000원 이상 5,000원 미만
        price = int(price) # 1원 단위
    elif price >= 100:  # 100원 이상 1,000원 미만
        price = int(price) # 1원 단위
    elif price >= 10:  # 10원 이상 100원 미만
        price = int(price*10)/10 # 0.1원 단위
    ...
    elif price >= 0.00001:  # 0.00001 이상 0.0001 미만
        price = int(price*10000000)/10000000 # 0.0000001원 단위
    elif price > 0:  # 0.00001 미만
...
class InitUpbitAdaptor:
    def __init__(self, my_upbit_access_key=None, my_upbit_secret_key=None, logging_dir=None):
        ...
        self.upbit_plug_logger = TradeCoreLogger("upbit_plug", logging_dir).logger
...
class UserUpbitAdaptor:
    def __init__(self, admin_id, acw_api, postgres_db_dict=None, market_code_combination=None, margin_liquidation_call_trade_queue=None, logging_dir=None):
        ...
        if market_code_combination is not None:
            ...
            self.logger = TradeCoreLogger(f"user_upbit_plug_{logger_name}", logging_dir).logger
            ...
            self.logger = TradeCoreLogger(f"user_upbit_plug", logging_dir).logger
