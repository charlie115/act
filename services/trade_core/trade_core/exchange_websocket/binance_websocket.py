...
def binance_websocket(stream_data_type, data, error_event, proc_name, market_type, logging_dir, acw_api, admin_id, inactivity_time_secs=120):
    ...
    logger = TradeCoreLogger(f"binance_{market_type.lower()}_{stream_data_type}_websocket", logging_dir).logger
...
def liquidation_websocket(liquidation_list, error_event, market_type, logging_dir, acw_api, admin_id):
    websocket_logger = TradeCoreLogger(f"binance_{market_type.lower()}_websocket", logging_dir).logger
...
class BinanceWebsocket:
    def __init__(self, admin_id, node, proc_n, get_symbol_list, acw_api, market_type, logging_dir):
        ...
        self.logger = TradeCoreLogger(f"binance_{self.market_type.lower()}_websocket", logging_dir).logger
        ...
        wait_for_market_ready(
            ...
            ("orderbook", "ticker"),
            self.logger,
            f"[BINANCE {self.market_type}] Waiting for websocket data to be loaded...",
        )
    ...
    def terminate_websocket(self):
        terminate_process_group(
            self,
            self.logger,
            f"[BINANCE {self.market_type}] All websockets' events have been set.",
        )
