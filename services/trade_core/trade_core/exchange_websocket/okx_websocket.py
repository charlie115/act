...
def init_websocket(stream_data_type, url, data, error_event, proc_name, market_type, logging_dir, acw_api, admin_id, inactivity_time_secs=60):
    ...
    logger = TradeCoreLogger(f"okx_{market_type.lower()}_websocket", logging_dir).logger
...
class OkxWebsocket:
    def __init__(self, admin_id, node, proc_n, get_symbol_list, acw_api, market_type, logging_dir=None):
        ...
        wait_for_market_ready(
            ...
            ("ticker",),
            self.logger,
            f"[OKX {self.market_type}]waiting for websocket data to be loaded..",
        )
    ...
    def terminate_websocket(self):
        terminate_process_group(
            self,
            self.logger,
            f"[OKX {self.market_type}]all websockets' event has been set",
        )
