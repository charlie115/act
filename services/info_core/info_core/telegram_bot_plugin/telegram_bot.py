...
class InitTelegramBot:
    def __init__(self, bot_token, logging_dir, node, redis_dict, db_dict, core, register_monitor_msg, admin_id_list, total_enabled_market_klines):
        self.node = node
        ...
        self.telegram_bot_logger = InfoCoreLogger("telegram_bot_logger", logging_dir).logger
        ...
        self.redis_client_db0 = RedisHelper(**redis_dict, db=0)
...
######### telegram chat functions####################################################################################################
    ...
    def cancel_server_check(self, update, context):
        def cancel_server_check_thread():
            ...
            if user_id in self.admin_id_list:
                try:
                    registered_server_check_list = [x.decode('utf-8') for x in self.redis_client_db0.get_all_keys() if 'INFO_CORE|SERVER_CHECK' in x.decode('utf-8')]
                    if context.args == []:
