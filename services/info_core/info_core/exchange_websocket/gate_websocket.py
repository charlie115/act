...
class GateWebsocket:
    ...
    def __init__(self, admin_id, node, proc_n, get_symbol_list, acw_api, market_type, logging_dir=None):
        ...
        wait_for_market_ready(
            ...
            f"[GATE {self.market_type}] Waiting for websocket data to be loaded...",
        )
                ...
                for proc_name, proc in list(self.websocket_proc_dict.items()):
                    ...
                    if result["action"] == "restart":
                        if result["reason"] == "heartbeat":
                            ...
                            content = (
                                f"[GATE {self.market_type}] {proc_name} => "
                                f"All {result['stale_count']}/{result['total_symbols']} symbols are stale for > {stale_threshold_secs}s. "
                                f"Forcing process restart."
                            )
                        else:
                            content = (
