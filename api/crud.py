...
user_uuid_for_wallet = config['node_settings'][node]['user_uuid_for_wallet']
acw_api = AcwApi()
...
data_processor = InitDataProcessor(logging_dir, InitDBClient(**mongo_db_dict))
...
async def create_trade(trade: schemas.TradeCreate, db: AsyncSession):
    ...
    if trade.trade_capital is not None and trade.trade_capital > 0:
        ...
        if target_market_type != "SPOT":
            ...
            full_content = title + '\n' + content
            acw_api.create_message_thread(trade_config.telegram_id, title, node, 'info', full_content)
...
async def fetch_pboundary(market_code_combination: str, base_asset: str, usdt_conversion: bool, percent_gap: float, interval: str, kline_num: int, draw_plot: bool, return_dict):
    try:
        output_dict = data_processor.get_pboundary(market_code_combination, base_asset, usdt_conversion, interval, kline_num, percent_gap, draw_plot, return_dict)
    except Exception as e:
