...
class Bybit:
    ...
    def usd_m_exchange_info(self):
        ...
        info_df.loc[:, 'perpetual'] = info_df['perpetual'].replace('LinearPerpetual', True).replace('LinearFutures', False)
        return info_df
    ...
    def coin_m_exchange_info(self):
        ...
        info_df = info_df.join(info_df['lotSizeFilter'].apply(pd.Series))
        info_df = info_df.join(info_df['priceFilter'].apply(pd.Series)).drop(columns=['leverageFilter','lotSizeFilter', 'priceFilter'], axis=1)
        info_df.loc[:, 'perpetual'] = info_df['perpetual'].replace('InversePerpetual', True).replace('InverseFutures', False)
        return info_df
...
class InitBybitAdaptor:
    def __init__(self, my_access_key=None, my_secret_key=None, info_dict={}, logging_dir=None):
        ...
        self.bybit_plug_logger = KimpBotLogger("bybit_plug", logging_dir).logger
    ...
    def get_fundingrate(self, futures_type='USD_M'):
        ...
        funding_df['perpetual'] = funding_df['fundingRate'].apply(lambda x: True if x != '' else False)
        funding_df.loc[:, ['fundingRate','nextFundingTime']] = funding_df.loc[:, ['fundingRate','nextFundingTime']].apply(pd.to_numeric, errors='coerce')
        funding_df['nextFundingTime'] = funding_df['nextFundingTime'].apply(lambda x: datetime.datetime.fromtimestamp(x/1000, tz=datetime.timezone.utc))
