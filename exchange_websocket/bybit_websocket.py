    ...
    def init_ticker_websocket(self, ticker_dict, symbol_list, error_event):
        ...
        def handle_message(message):
            ...
            if 'data' in message.keys():
                ticker_dict[message['data']['symbol']] = {**message['data'], 'last_update_timestamp': int(datetime.datetime.utcnow().timestamp()*1000000)}
                # self.redis_client_db1.set_data(f"INFO_CORE|BYBIT_{self.market_type}|ticker|{message['data']['symbol']}", json.dumps({**message['data'], "last_update_timestamp": int(datetime.datetime.utcnow().timestamp() * 1000000)}))
                # self.redis_client_db1.set_data(f"INFO_CORE|BYBIT_{self.market_type}|ticker", pickle.dumps(dict(ticker_dict)))
    ...
    def init_orderbook_websocket(self, orderbook_dict, symbol_list, error_event):
        ...
        def handle_message(message):
            if error_event.is_set():
                ...
                return
            if 'data' in message.keys():
                orderbook_dict[message['data']['s']] = {**message['data'], 'last_update_timestamp': int(datetime.datetime.utcnow().timestamp()*1000000)}
                # self.redis_client_db1.set_data(f"INFO_CORE|BYBIT_{self.market_type}|orderbook|{message['data']['s']}", json.dumps({**message['data'], "last_update_timestamp": int(datetime.datetime.utcnow().timestamp() * 1000000)}))
                # self.redis_client_db1.set_data(f"INFO_CORE|BYBIT_{self.market_type}|orderbook", pickle.dumps(dict(orderbook_dict)))
        if self.market_type == "SPOT":
            for symbol_bunch in cut_list(symbol_list):
                ws.orderbook_stream(
                    depth=1,
                ...
                if sorted(self.before_symbols_list) != sorted(new_symbols_list):
                    ...
                    for each_shared_symbol in deleted_shared_symbol:
                        ...
                        try:
                            ...
                            # self.redis_client_db1.redis_conn.delete(f"INFO_CORE|BYBIT_{self.market_type}|ticker|{each_shared_symbol}")
                        except Exception:
                            pass
                        try:
                            del self.orderbook_dict[each_shared_symbol]
                            # self.redis_client_db1.redis_conn.delete(f"INFO_CORE|BYBIT_{self.market_type}|orderbook|{each_shared_symbol}")
                        except Exception:
                            pass
