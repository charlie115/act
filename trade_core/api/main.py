...
async def fetch_exit_trade(request: schemas.TradeUUIDRequest = Body(...), db: AsyncSession = Depends(crud.get_db)):
    return await crud.exit_trade(str(request.trade_uuid), db)